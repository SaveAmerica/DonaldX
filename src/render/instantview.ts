...
const generateSummary = async(c: Context, thread?: SocialThread) => {
  if (!c.env.AI || !thread?.thread) {
    return '';
  }

  const summary = await summarizeThread(c, thread);

  if (summary) {
    return `<table>
      <thead>
        <th><b>${i18next.t('threadSummary')}</b></th>
      </thead>
      <tbody>
        <th>${summary}</th>
      </tbody>
    ...
    return `<table>
      <thead>
        <th><b>${i18next.t('summaryFailed')}</b></th>
      </thead>
    </table>`;
   }
}
